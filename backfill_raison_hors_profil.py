#!/usr/bin/env python3
"""One-shot migration: writes "Hors profil" into column R (Raison_exclusion)
for rows whose title matches a blacklist term (extract/filters.is_blacklisted)
but whose Raison_exclusion is currently empty - offers imported before
extract_eml.py auto-detected this criterion.
...
Usage:
    python3 backfill_raison_hors_profil.py <sheet_name> [--apply]
...
from extract.filters import is_blacklisted
...
def rows_to_fill(
    ...
    """1-indexed row numbers whose title matches a blacklist term and whose
    ...
    for i, titre in enumerate(titre_values):
        ...
        if not raison and is_blacklisted(titre, terms):
...
def run(sheet_name: str, apply: bool) -> None:
    ...
    terms = config.get("blacklist_titres", [])
